...
struct QuadratureEncoder {
    QuadratureEncoder( gpio_num_t aPin, gpio_num_t bPin, gpio_num_t btnPin = GPIO_NUM_MAX, Worker *worker = nullptr )
        : _value( 0 ), _aPin( aPin ), _bPin( bPin ), _btnPin( btnPin ), _worker( worker )
    {
        _lastTick = millis();
    }
    ...
    void start() {
        ...
        gpio_set_intr_type( _btnPin, GPIO_INTR_ANYEDGE );
